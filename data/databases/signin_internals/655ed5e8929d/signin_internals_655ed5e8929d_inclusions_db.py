{
    'config': {
        'evaluated_repo_root':
            '/usr/local/google/home/blundell/clankium/src',
        'included_files': [
            'components/signin/core/browser/account_consistency_method.cc',
            'components/signin/core/browser/account_consistency_method.h',
            'components/signin/core/browser/account_tracker_service.cc',
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/device_id_helper.cc',
            'components/signin/core/browser/device_id_helper.h',
            'components/signin/core/browser/gaia_cookie_manager_service.cc',
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.cc',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.cc',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_error_controller.cc',
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_internals_util.cc',
            'components/signin/core/browser/signin_internals_util.h',
            'components/signin/core/browser/signin_manager.cc',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager_base.cc',
            'components/signin/core/browser/signin_manager_base.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.cc',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.cc',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.cc',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.cc',
            'components/signin/core/browser/test_signin_client.h',
            'chrome/browser/signin/account_tracker_service_factory.cc',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client.cc',
            'chrome/browser/signin/chrome_signin_client.h',
            'chrome/browser/signin/chrome_signin_client_factory.cc',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.cc',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.cc',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/signin/fake_signin_manager_builder.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.cc',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/ios_chrome_signin_client.cc',
            'ios/chrome/browser/signin/ios_chrome_signin_client.h',
            'ios/chrome/browser/signin/chrome_signin_client_factory.cc',
            'ios/chrome/browser/signin/chrome_signin_client_factory.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'ios/chrome/browser/signin/profile_oauth2_token_service_factory.mm',
            'ios/chrome/browser/signin/signin_error_controller_factory.cc',
            'ios/chrome/browser/signin/signin_error_controller_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.cc',
            'ios/chrome/browser/signin/signin_manager_factory.h',
            'ios/chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'ios/chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.h',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm',
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'ios/chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'name':
            'signin_internals',
        'repo_root':
            'CHROMIUM_ROOT'
    },
    'included_to_including': {
        'chrome/browser/signin/account_tracker_service_factory.h': [
            'chrome/browser/signin/fake_account_fetcher_service_builder.cc',
            'chrome/browser/profiles/gaia_info_update_service_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/ui/ash/multi_user/multi_user_util_chromeos_unittest.cc',
            'chrome/browser/android/profiles/profile_downloader_android.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service_factory.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/account_fetcher_service_factory.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'chrome/browser/autocomplete/search_provider_unittest.cc',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'chrome/browser/android/signin/account_tracker_service_android.cc',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/profiles/profile_manager.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service.cc',
            'chrome/browser/autofill/personal_data_manager_factory.cc',
            'chrome/browser/signin/signin_ui_util.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/chromeos/login/users/avatar/user_image_manager_browsertest.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'chrome/browser/profiles/chrome_browser_main_extra_parts_profiles.cc',
            'chrome/browser/profiles/profile_impl.cc',
            'chrome/browser/signin/account_tracker_service_factory.cc',
            'chrome/browser/profiles/profile_window.cc',
            'chrome/browser/ui/webui/supervised_user_internals_message_handler.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler_unittest.cc',
            'chrome/browser/signin/signin_promo.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'chrome/browser/ui/webui/signin/signin_dice_internals_handler.cc',
            'chrome/browser/android/signin/signin_investigator_android.cc'
        ],
        'chrome/browser/signin/chrome_signin_client.h': [
            'chrome/browser/signin/chrome_signin_helper.cc',
            'chrome/browser/signin/chrome_signin_client.cc',
            'chrome/browser/signin/chrome_signin_client_unittest.cc',
            'chrome/browser/signin/chrome_signin_client_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_client_factory.h': [
            'chrome/browser/signin/fake_account_fetcher_service_builder.cc',
            'chrome/browser/profiles/gaia_info_update_service_unittest.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/account_fetcher_service_factory.cc',
            'chrome/browser/signin/chrome_signin_helper.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'chrome/browser/chromeos/login/signin/device_id_browsertest.cc',
            'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/signin/chrome_signin_client_unittest.cc',
            'chrome/browser/signin/account_reconcilor_factory.cc',
            'chrome/browser/signin/account_tracker_service_factory.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'chrome/browser/signin/chrome_signin_client_factory.cc'
        ],
        'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h': [
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc'
        ],
        'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h': [
            'chrome/browser/chromeos/arc/arc_session_manager_browsertest.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'chrome/browser/supervised_user/supervised_user_service_unittest.cc'
        ],
        'chrome/browser/signin/fake_signin_manager_builder.h': [
            'chrome/browser/ui/sync/sync_promo_ui_unittest.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler_unittest.cc',
            'chrome/browser/extensions/api/sync_file_system/sync_file_system_browsertest.cc',
            'chrome/browser/ui/ash/multi_user/multi_user_util_chromeos_unittest.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'chrome/browser/ui/toolbar/media_router_contextual_menu_unittest.cc',
            'chrome/browser/password_manager/password_store_signin_notifier_impl_unittest.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/ui/webui/settings/people_handler_unittest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/signin/signin_error_notifier_ash_unittest.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_browsertest.cc',
            'chrome/browser/ui/search/search_tab_helper_unittest.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler_unittest.cc',
            'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_unittest.cc',
            'chrome/browser/permissions/permission_uma_util_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler_unittest.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc'
        ],
        'chrome/browser/signin/gaia_cookie_manager_service_factory.h': [
            'chrome/browser/search/one_google_bar/one_google_bar_service_factory.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/signin/account_investigator_factory.cc',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'chrome/browser/profiles/profile_manager.cc',
            'chrome/browser/signin/signin_tracker_factory.cc',
            'chrome/browser/ui/search/local_ntp_browsertest.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager_factory.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'chrome/browser/signin/account_reconcilor_factory.cc',
            'chrome/browser/ui/webui/signin_internals_ui.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc'
        ],
        'chrome/browser/signin/profile_oauth2_token_service_factory.h': [
            'chrome/browser/signin/fake_account_fetcher_service_builder.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_factory.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'chrome/browser/chromeos/arc/arc_session_manager_browsertest.cc',
            'chrome/browser/apps/drive/drive_service_bridge.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_misc.cc',
            'chrome/browser/sync/test/integration/sync_test.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/profiles/profiles_state.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/gcm/gcm_profile_service_unittest.cc',
            'chrome/browser/autocomplete/contextual_suggestions_service_factory.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/force_signin_verifier.cc',
            'chrome/browser/sync/test/integration/sync_auth_test.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service_factory.cc',
            'chrome/browser/signin/account_fetcher_service_factory.cc',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'chrome/browser/supervised_user/supervised_user_service.cc',
            'chrome/browser/history/web_history_service_factory.cc',
            'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service_factory.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_context.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/extensions/external_provider_impl_chromeos_unittest.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_observer_unittest.cc',
            'chrome/browser/signin/oauth2_token_service_delegate_android.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'chrome/browser/ui/app_list/speech_auth_helper.cc',
            'chrome/browser/browsing_data/sync_aware_counter_browsertest.cc',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'chrome/browser/extensions/api/hotword_private/hotword_private_apitest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'chrome/browser/sync/test/integration/profile_sync_service_harness.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_mobile.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder_factory.cc',
            'chrome/browser/signin/chrome_signin_client.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service.cc',
            'chrome/browser/ntp_snippets/contextual_content_suggestions_service_factory.cc',
            'chrome/browser/ui/toolbar/app_menu_model_unittest.cc',
            'chrome/browser/signin/signin_tracker_factory.cc',
            'chrome/browser/invalidation/profile_invalidation_provider_factory.cc',
            'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc',
            'chrome/browser/signin/easy_unlock_service.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc',
            'chrome/browser/gcm/gcm_profile_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager_factory.cc',
            'chrome/browser/sync/profile_sync_test_util.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/search/suggestions/suggestions_service_factory.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'chrome/browser/chromeos/login/signin/token_handle_fetcher.cc',
            'chrome/browser/chromeos/drive/drive_integration_service.cc',
            'chrome/browser/ntp_snippets/content_suggestions_service_factory.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/extensions/extension_gcm_app_handler_unittest.cc',
            'chrome/browser/signin/easy_unlock_service_regular.cc',
            'chrome/browser/sync_file_system/drive_backend/sync_engine.cc',
            'chrome/browser/signin/account_reconcilor_factory.cc',
            'chrome/browser/chromeos/app_mode/startup_app_launcher.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_drive.cc',
            'chrome/browser/extensions/updater/chrome_extension_downloader_factory.cc',
            'chrome/browser/profiles/profile_impl.cc',
            'chrome/browser/supervised_user/supervised_user_service_factory.cc',
            'chrome/browser/ui/views/payments/cvc_unmask_view_controller.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc',
            'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'chrome/browser/ui/autofill/chrome_autofill_client.cc',
            'chrome/browser/ui/webui/signin/signin_dice_internals_handler.cc',
            'chrome/browser/feedback/feedback_uploader_chrome.cc',
            'chrome/browser/signin/chrome_proximity_auth_client.cc',
            'chrome/browser/supervised_user/supervised_user_service_unittest.cc',
            'chrome/browser/browsing_data/history_counter_browsertest.cc',
            'chrome/browser/sync/chrome_sync_client.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.cc'
        ],
        'chrome/browser/signin/signin_error_controller_factory.h': [
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler_unittest.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/signin/signin_global_error_factory.cc',
            'chrome/browser/profiles/profiles_state.cc',
            'chrome/browser/chromeos/login/signin/auth_sync_observer.cc',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util_unittest.cc',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'chrome/browser/ui/webui/settings/people_handler_unittest.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm',
            'chrome/browser/signin/signin_error_notifier_ash_unittest.cc',
            'chrome/browser/signin/signin_error_notifier_factory_ash.cc',
            'chrome/browser/signin/signin_ui_util.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'chrome/browser/signin/chrome_signin_client_unittest.cc',
            'chrome/browser/sync/sync_ui_util.cc',
            'chrome/browser/app_controller_mac_unittest.mm',
            'chrome/browser/chromeos/login/signin/auth_sync_observer_factory.cc',
            'chrome/browser/ui/webui/settings/people_handler.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler_unittest.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler.cc',
            'chrome/browser/signin/signin_promo.cc',
            'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc',
            'chrome/browser/signin/chrome_signin_client_factory.cc',
            'chrome/browser/signin/signin_error_controller_factory.cc'
        ],
        'chrome/browser/signin/signin_manager_factory.h': [
            'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api_unittest.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_factory.cc',
            'chrome/browser/profiles/gaia_info_update_service_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler.cc',
            'chrome/browser/android/bookmarks/bookmark_bridge.cc',
            'chrome/browser/extensions/extension_system_factory.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'chrome/browser/ui/sync/sync_promo_ui_unittest.cc',
            'chrome/browser/chromeos/policy/device_local_account_browsertest.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler_unittest.cc',
            'chrome/browser/ui/webui/print_preview/privet_printer_handler.cc',
            'chrome/browser/apps/drive/drive_service_bridge.cc',
            'chrome/browser/extensions/api/feedback_private/chrome_feedback_private_delegate.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_misc.cc',
            'chrome/browser/sync/test/integration/sync_test.cc',
            'chrome/browser/chrome_browser_main_android.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_sync_service.cc',
            'chrome/browser/ui/webui/profile_info_watcher.cc',
            'chrome/browser/ui/ash/multi_user/multi_user_util_chromeos_unittest.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/gcm/gcm_profile_service_unittest.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service_factory.cc',
            'chrome/browser/chromeos/login/signin/auth_sync_observer.cc',
            'chrome/browser/autocomplete/contextual_suggestions_service_factory.cc',
            'chrome/browser/ui/search/search_tab_helper.cc',
            'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/chromeos/login/lock/screen_locker.cc',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/force_signin_verifier.cc',
            'chrome/browser/password_manager/password_store_signin_notifier_impl.cc',
            'chrome/browser/signin/easy_unlock_service_unittest_chromeos.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service_factory.cc',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'chrome/browser/first_run/first_run.cc',
            'chrome/browser/supervised_user/supervised_user_service.cc',
            'chrome/browser/ui/views/profiles/avatar_button.cc',
            'chrome/browser/ui/toolbar/media_router_contextual_menu_unittest.cc',
            'chrome/browser/history/web_history_service_factory.cc',
            'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service_factory.cc',
            'chrome/browser/password_manager/password_store_signin_notifier_impl_unittest.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_context.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/extensions/external_provider_impl_chromeos_unittest.cc',
            'chrome/browser/ui/webui/settings/settings_manage_profile_handler.cc',
            'chrome/browser/prefs/browser_prefs.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'chrome/browser/extensions/external_component_loader.cc',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/extensions/api/signed_in_devices/signed_in_devices_manager_unittest.cc',
            'chrome/browser/ui/views/profiles/forced_reauthentication_dialog_view_browsertest.cc',
            'chrome/browser/extensions/api/enterprise_platform_keys/enterprise_platform_keys_api_unittest.cc',
            'chrome/browser/signin/fake_signin_manager_builder.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_observer_unittest.cc',
            'chrome/browser/ui/passwords/password_manager_presenter.cc',
            'chrome/browser/ui/app_list/speech_auth_helper.cc',
            'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util_unittest.cc',
            'chrome/browser/signin/account_investigator_factory.cc',
            'chrome/browser/autocomplete/search_provider_unittest.cc',
            'chrome/browser/signin/process_dice_header_observer_impl.cc',
            'chrome/browser/signin/chrome_signin_status_metrics_provider_delegate.h',
            'chrome/browser/extensions/api/screenlock_private/screenlock_private_apitest.cc',
            'chrome/browser/browsing_data/sync_aware_counter_browsertest.cc',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'chrome/browser/policy/cloud/cloud_policy_manager_browsertest.cc',
            'chrome/browser/ui/webui/settings/people_handler_unittest.cc',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'chrome/browser/ui/cocoa/bookmarks/bookmark_bubble_controller_unittest.mm',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'chrome/test/data/webui/ntp4_browsertest.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/ui/webui/settings/settings_clear_browsing_data_handler.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'chrome/browser/android/data_usage/data_use_ui_tab_model_factory.cc',
            'chrome/browser/extensions/api/hotword_private/hotword_private_apitest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'chrome/browser/signin/signin_error_notifier_ash_unittest.cc',
            'chrome/browser/app_controller_mac.mm',
            'chrome/browser/profiles/profile_manager.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service.cc',
            'chrome/browser/ui/toolbar/app_menu_model.cc',
            'chrome/browser/chromeos/login/session/chrome_session_manager.cc',
            'chrome/browser/password_manager/password_store_factory.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder_factory.cc',
            'chrome/browser/ui/extensions/extension_installed_bubble_browsertest.cc',
            'chrome/browser/signin/chrome_signin_client.cc',
            'chrome/browser/autofill/personal_data_manager_factory.cc',
            'chrome/browser/signin/signin_ui_util.cc',
            'chrome/browser/ntp_snippets/contextual_content_suggestions_service_factory.cc',
            'chrome/browser/policy/cloud/component_cloud_policy_browsertest.cc',
            'chrome/browser/password_manager/chrome_password_manager_client.cc',
            'chrome/browser/signin/signin_tracker_factory.cc',
            'chrome/browser/invalidation/profile_invalidation_provider_factory.cc',
            'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc',
            'chrome/browser/signin/easy_unlock_service.cc',
            'chrome/browser/ui/chrome_pages.cc',
            'chrome/browser/ui/toolbar/media_router_contextual_menu.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc',
            'chrome/browser/gcm/gcm_profile_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager_factory.cc',
            'chrome/browser/chromeos/first_run/first_run.cc',
            'chrome/browser/supervised_user/supervised_user_google_auth_navigation_throttle.cc',
            'chrome/browser/sync/profile_sync_test_util.cc',
            'chrome/browser/profiles/profile_window_browsertest.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/search/suggestions/suggestions_service_factory.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service.cc',
            'chrome/browser/chromeos/login/kiosk_browsertest.cc',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'chrome/browser/sync/profile_sync_service_android.cc',
            'chrome/browser/payments/chrome_payment_request_delegate.cc',
            'chrome/browser/chromeos/login/signin/token_handle_fetcher.cc',
            'chrome/browser/ui/webui/ntp/ntp_resource_cache.cc',
            'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api.cc',
            'chrome/browser/chromeos/drive/drive_integration_service.cc',
            'chrome/browser/ntp_snippets/content_suggestions_service_factory.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_base.cc',
            'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_browsertest.cc',
            'chrome/browser/download/notification/download_notification_browsertest.cc',
            'chrome/browser/signin/signin_global_error.cc',
            'chrome/browser/extensions/extension_gcm_app_handler_unittest.cc',
            'chrome/browser/sync/sync_ui_util.cc',
            'chrome/browser/profiles/chrome_browser_main_extra_parts_profiles.cc',
            'chrome/browser/extensions/api/webstore_private/webstore_private_api.cc',
            'chrome/browser/policy/cloud/cloud_policy_browsertest.cc',
            'chrome/browser/printing/cloud_print/privet_notifications.cc',
            'chrome/browser/ui/webui/app_launcher_login_handler.cc',
            'chrome/browser/ui/startup/startup_tab_provider.cc',
            'chrome/browser/signin/easy_unlock_service_regular.cc',
            'chrome/browser/sync_file_system/drive_backend/sync_engine.cc',
            'chrome/browser/signin/account_reconcilor_factory.cc',
            'chrome/browser/signin/signin_promo_util.cc',
            'chrome/browser/app_controller_mac_unittest.mm',
            'chrome/browser/chromeos/app_mode/startup_app_launcher.cc',
            'chrome/browser/safe_browsing/permission_reporter_browsertest.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_drive.cc',
            'chrome/browser/ui/webui/settings/people_handler.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler.cc',
            'chrome/browser/extensions/updater/chrome_extension_downloader_factory.cc',
            'chrome/browser/ui/search/search_tab_helper_unittest.cc',
            'chrome/browser/profiles/profile_impl.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler_unittest.cc',
            'chrome/browser/profiles/profile_window.cc',
            'chrome/browser/chromeos/file_manager/file_manager_browsertest.cc',
            'chrome/browser/ui/views/payments/cvc_unmask_view_controller.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc',
            'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_unittest.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_sync_service_factory.cc',
            'chrome/browser/ui/webui/media_router/media_router_webui_message_handler.cc',
            'chrome/browser/metrics/ukm_browsertest.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler.cc',
            'chrome/browser/permissions/permission_uma_util_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler_unittest.cc',
            'chrome/browser/signin/signin_promo.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'chrome/browser/ui/autofill/chrome_autofill_client.cc',
            'chrome/browser/ui/webui/signin/signin_dice_internals_handler.cc',
            'chrome/browser/ui/webui/welcome_handler.cc',
            'chrome/browser/profiles/gaia_info_update_service_factory.cc',
            'chrome/browser/feedback/feedback_uploader_chrome.cc',
            'chrome/browser/signin/chrome_proximity_auth_client.cc',
            'chrome/browser/profiles/gaia_info_update_service.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter.cc',
            'chrome/browser/supervised_user/supervised_user_service_unittest.cc',
            'chrome/browser/browsing_data/history_counter_browsertest.cc',
            'chrome/browser/ui/webui/settings/search_engines_handler.cc',
            'chrome/browser/ui/webui/ntp/ntp_resource_cache_factory.cc',
            'chrome/browser/ui/webui/md_history_ui.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.cc'
        ],
        'components/signin/core/browser/account_tracker_service.h': [
            'chrome/browser/supervised_user/child_accounts/child_account_service.h',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler.h',
            'chrome/browser/signin/oauth2_token_service_delegate_android.h',
            'components/signin/core/browser/child_account_info_fetcher_impl.cc',
            'chrome/browser/profiles/gaia_info_update_service_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'components/signin/core/browser/account_fetcher_service.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.h',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_unittest.cc',
            'components/autofill/core/browser/autofill_test_utils.cc',
            'chrome/browser/ui/ash/multi_user/multi_user_util_chromeos_unittest.cc',
            'chrome/browser/android/profiles/profile_downloader_android.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'components/browser_sync/profile_sync_service_startup_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'services/identity/identity_manager.cc',
            'components/signin/core/browser/signin_manager_base.cc',
            'components/signin/core/browser/fake_signin_manager.cc',
            'components/signin/ios/browser/account_consistency_service_unittest.mm',
            'components/suggestions/suggestions_service_impl_unittest.cc',
            'ios/chrome/browser/ui/settings/sync_settings_collection_view_controller.mm',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'components/signin/core/browser/about_signin_internals.cc',
            'components/signin/core/browser/fake_account_fetcher_service.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'chrome/browser/autocomplete/search_provider_unittest.cc',
            'components/browsing_data/core/history_notice_utils_unittest.cc',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'chrome/browser/android/signin/account_tracker_service_android.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'ios/chrome/browser/ui/authentication/signed_in_accounts_view_controller.mm',
            'chrome/browser/profiles/profile_manager.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service.cc',
            'components/browser_sync/profile_sync_service_unittest.cc',
            'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm',
            'chrome/browser/autofill/personal_data_manager_factory.cc',
            'chrome/browser/signin/signin_ui_util.cc',
            'components/autofill/core/browser/autofill_metrics_unittest.cc',
            'components/signin/core/browser/account_tracker_service_unittest.cc',
            'ios/web_view/internal/signin/cwv_authentication_controller.mm',
            'components/autofill/core/browser/personal_data_manager.cc',
            'ios/web_view/internal/signin/web_view_account_tracker_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'chrome/browser/chromeos/first_run/first_run.cc',
            'services/identity/identity_manager_unittest.cc',
            'components/password_manager/sync/browser/sync_username_test_base.h',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'components/browser_sync/profile_sync_test_util.h',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'components/ntp_snippets/remote/remote_suggestions_status_service_impl_unittest.cc',
            'chrome/browser/profiles/profile_downloader.h',
            'components/signin/core/browser/account_investigator_unittest.cc',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'components/autofill/core/browser/personal_data_manager_unittest.cc',
            'ios/chrome/test/app/signin_test_util.mm',
            'components/signin/core/browser/access_token_fetcher_unittest.cc',
            'chrome/browser/signin/account_tracker_service_factory.cc',
            'chrome/browser/search/one_google_bar/one_google_bar_service_unittest.cc',
            'chrome/browser/profiles/profile_window.cc',
            'components/signin/core/browser/signin_manager.cc',
            'chrome/browser/ui/webui/supervised_user_internals_message_handler.cc',
            'components/ntp_snippets/remote/test_utils.cc',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate.mm',
            'chrome/browser/signin/signin_promo.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'components/signin/core/browser/gaia_cookie_manager_service_unittest.cc',
            'ios/chrome/browser/signin/account_tracker_service_factory.cc',
            'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc',
            'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'components/signin/core/browser/child_account_info_fetcher_android.cc',
            'chrome/browser/signin/dice_response_handler_unittest.cc',
            'ios/chrome/browser/ui/authentication/authentication_flow_performer.mm',
            'components/history/core/browser/web_history_service_unittest.cc',
            'components/signin/core/browser/gaia_cookie_manager_service.cc',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate_unittest.mm',
            'chrome/browser/android/signin/signin_investigator_android.cc',
            'ios/chrome/browser/signin/authentication_service.mm',
            'components/signin/core/browser/account_tracker_service.cc'
        ],
        'components/signin/core/browser/fake_gaia_cookie_manager_service.h': [
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'components/signin/core/browser/account_investigator_unittest.cc',
            'chrome/browser/search/one_google_bar/one_google_bar_service_unittest.cc'
        ],
        'components/signin/core/browser/fake_profile_oauth2_token_service.h': [
            'components/ntp_snippets/contextual/contextual_suggestions_fetcher_impl_unittest.cc',
            'chrome/browser/chromeos/arc/arc_session_manager_browsertest.cc',
            'components/invalidation/impl/fake_invalidation_service.h',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_unittest.cc',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'components/browser_sync/profile_sync_service_startup_unittest.cc',
            'chrome/browser/chromeos/policy/android_management_client_unittest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'components/ntp_snippets/remote/remote_suggestions_provider_impl_unittest.cc',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'components/suggestions/suggestions_service_impl_unittest.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'components/browsing_data/core/history_notice_utils_unittest.cc',
            'chrome/browser/supervised_user/child_accounts/family_info_fetcher_unittest.cc',
            'components/ntp_snippets/breaking_news/subscription_manager_impl_unittest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'components/invalidation/impl/gcm_invalidation_bridge_unittest.cc',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary_unittest.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc',
            'chrome/browser/sync/test/integration/fake_server_invalidation_service.h',
            'services/identity/identity_manager_unittest.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'components/browser_sync/profile_sync_test_util.h',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'components/ntp_snippets/remote/remote_suggestions_fetcher_impl_unittest.cc',
            'components/signin/core/browser/access_token_fetcher_unittest.cc',
            'components/ntp_snippets/remote/test_utils.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_refresh_token_fetcher_unittest.cc',
            'components/history/core/browser/web_history_service_unittest.cc',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter_unittest.cc',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm',
            'chrome/browser/supervised_user/supervised_user_service_unittest.cc',
            'components/signin/core/browser/fake_profile_oauth2_token_service.cc'
        ],
        'components/signin/core/browser/fake_signin_manager.h': [
            'components/ntp_snippets/contextual/contextual_suggestions_fetcher_impl_unittest.cc',
            'chrome/browser/signin/fake_signin_manager_builder.h',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_unittest.cc',
            'components/ntp_snippets/remote/remote_suggestions_provider_impl_unittest.cc',
            'components/ntp_snippets/remote/test_utils.h',
            'components/signin/core/browser/fake_signin_manager.cc',
            'chrome/browser/ui/webui/browsing_history_handler_unittest.cc',
            'components/signin/ios/browser/account_consistency_service_unittest.mm',
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'components/suggestions/suggestions_service_impl_unittest.cc',
            'components/browsing_data/core/history_notice_utils_unittest.cc',
            'components/ntp_snippets/breaking_news/subscription_manager_impl_unittest.cc',
            'chrome/test/base/chrome_render_view_host_test_harness.cc',
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller_unittest.mm',
            'components/browser_sync/profile_sync_service_unittest.cc',
            'components/autofill/core/browser/autofill_metrics_unittest.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'services/identity/identity_manager_unittest.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc',
            'components/browser_sync/profile_sync_test_util.h',
            'components/ntp_snippets/remote/remote_suggestions_status_service_impl_unittest.cc',
            'components/signin/core/browser/account_investigator_unittest.cc',
            'components/autofill/core/browser/personal_data_manager_unittest.cc',
            'components/ntp_snippets/remote/remote_suggestions_fetcher_impl_unittest.cc',
            'components/signin/core/browser/access_token_fetcher_unittest.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler_unittest.cc',
            'chrome/browser/signin/dice_response_handler_unittest.cc',
            'components/history/core/browser/web_history_service_unittest.cc'
        ],
        'components/signin/core/browser/gaia_cookie_manager_service.h': [
            'chrome/browser/supervised_user/child_accounts/child_account_service.h',
            'components/signin/core/browser/account_investigator.h',
            'components/browser_sync/profile_sync_service.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'components/signin/ios/browser/account_consistency_service_unittest.mm',
            'components/signin/ios/browser/merge_session_observer_bridge.h',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'components/signin/core/browser/about_signin_internals.h',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'components/signin/core/browser/account_reconcilor.h',
            'ios/web_view/internal/signin/web_view_gaia_cookie_manager_service_factory.cc',
            'chrome/browser/profiles/profile_manager.cc',
            'components/signin/ios/browser/account_consistency_service.h',
            'chrome/browser/search/one_google_bar/one_google_bar_service.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'components/signin/core/browser/signin_tracker.h',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'components/signin/core/browser/signin_tracker.cc',
            'components/signin/core/browser/signin_manager.cc',
            'chrome/browser/ui/webui/signin_internals_ui.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_verifier.h',
            'components/signin/core/browser/gaia_cookie_manager_service_unittest.cc',
            'ios/chrome/browser/ui/webui/signin_internals_ui_ios.cc',
            'components/signin/core/browser/gaia_cookie_manager_service.cc',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h'
        ],
        'components/signin/core/browser/profile_oauth2_token_service.h': [
            'chrome/browser/signin/fake_account_fetcher_service_builder.cc',
            'chrome/browser/signin/oauth2_token_service_delegate_android.h',
            'chrome/browser/chromeos/arc/auth/arc_background_auth_code_fetcher.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'chrome/browser/apps/drive/drive_service_bridge.cc',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.h',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_misc.cc',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_service_factory.cc',
            'chrome/browser/signin/profile_oauth2_token_service_factory.cc',
            'chrome/browser/signin/signin_global_error_factory.cc',
            'chrome/browser/profiles/profiles_state.cc',
            'chrome/browser/autocomplete/contextual_suggestions_service_factory.cc',
            'components/signin/core/browser/profile_identity_provider.cc',
            'components/browser_sync/profile_sync_service_startup_unittest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'components/ntp_snippets/remote/json_request.cc',
            'chrome/browser/sync/test/integration/sync_auth_test.cc',
            'chrome/browser/signin/account_fetcher_service_factory.cc',
            'chrome/browser/supervised_user/supervised_user_service.cc',
            'ios/chrome/browser/sync/ios_chrome_sync_client.mm',
            'chrome/browser/history/web_history_service_factory.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_context.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/extensions/external_provider_impl_chromeos_unittest.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'ios/chrome/browser/ui/settings/sync_settings_collection_view_controller.mm',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_factory.cc',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'components/signin/core/browser/about_signin_internals.cc',
            'components/signin/core/browser/fake_account_fetcher_service.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/ui/app_list/speech_auth_helper.cc',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/browsing_data/sync_aware_counter_browsertest.cc',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'components/signin/core/browser/profile_oauth2_token_service.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'ios/chrome/browser/suggestions/suggestions_service_factory.mm',
            'chrome/browser/extensions/api/hotword_private/hotword_private_apitest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'ios/web_view/internal/signin/web_view_account_fetcher_service_factory.cc',
            'ios/web_view/internal/signin/web_view_gaia_cookie_manager_service_factory.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate_unittest.cc',
            'chrome/browser/sync/test/integration/profile_sync_service_harness.cc',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.cc',
            'ios/chrome/browser/ui/authentication/signed_in_accounts_view_controller.mm',
            'chrome/browser/signin/signin_error_notifier_factory_ash.cc',
            'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm',
            'chrome/browser/policy/cloud/user_policy_signin_service_mobile.cc',
            'ios/chrome/browser/ui/settings/sync_encryption_passphrase_collection_view_controller.mm',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder_factory.cc',
            'chrome/browser/signin/chrome_signin_client.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service.cc',
            'chrome/browser/ui/app_list/speech_auth_helper.h',
            'chrome/browser/ntp_snippets/contextual_content_suggestions_service_factory.cc',
            'services/identity/identity_manager.h',
            'chrome/browser/invalidation/profile_invalidation_provider_factory.cc',
            'ios/web_view/internal/signin/cwv_authentication_controller.mm',
            'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc',
            'chrome/browser/signin/easy_unlock_service.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'ios/web_view/internal/signin/web_view_oauth2_token_service_factory.mm',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/search/suggestions/suggestions_service_factory.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service.cc',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'chrome/browser/chromeos/arc/policy/arc_android_management_checker.cc',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'ios/chrome/browser/signin/oauth2_token_service_factory.mm',
            'chrome/browser/chromeos/login/signin/token_handle_fetcher.cc',
            'chrome/browser/chromeos/drive/drive_integration_service.cc',
            'chrome/browser/ntp_snippets/content_suggestions_service_factory.cc',
            'chrome/browser/devtools/device/devtools_android_bridge.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/signin/easy_unlock_service_regular.cc',
            'chrome/browser/sync_file_system/drive_backend/sync_engine.cc',
            'chrome/browser/chromeos/app_mode/startup_app_launcher.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_drive.cc',
            'components/signin/core/browser/signin_tracker.cc',
            'components/browser_sync/profile_sync_service.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc',
            'components/signin/core/browser/account_reconcilor.cc',
            'ios/chrome/browser/signin/account_fetcher_service_factory.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc',
            'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc',
            'chrome/browser/signin/dice_response_handler_unittest.cc',
            'chrome/browser/feedback/feedback_uploader_chrome.cc',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_test_util.cc',
            'ios/chrome/browser/history/web_history_service_factory.cc',
            'chrome/browser/signin/chrome_proximity_auth_client.cc',
            'ios/chrome/browser/signin/authentication_service_unittest.mm',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate_unittest.mm',
            'chrome/browser/browsing_data/history_counter_browsertest.cc',
            'chrome/browser/sync/chrome_sync_client.cc',
            'ios/chrome/browser/signin/authentication_service.mm',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/invalidation/impl/gcm_invalidation_bridge.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.cc'
        ],
        'components/signin/core/browser/signin_client.h': [
            'components/signin/core/browser/child_account_info_fetcher_impl.cc',
            'components/signin/core/browser/account_fetcher_service.cc',
            'components/signin/core/browser/signin_manager_base.cc',
            'ios/web_view/internal/signin/web_view_signin_client_factory.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'components/signin/core/browser/about_signin_internals.cc',
            'components/signin/core/browser/about_signin_internals.h',
            'components/signin/core/browser/signin_cookie_changed_subscription.h',
            'components/signin/ios/browser/account_consistency_service.mm',
            'components/signin/core/browser/account_reconcilor.h',
            'components/signin/core/browser/signin_client.cc',
            'chrome/browser/signin/chrome_signin_client.h',
            'components/signin/ios/browser/ios_signin_client.h',
            'components/sync/driver/glue/sync_backend_host_impl.cc',
            'chrome/browser/chromeos/login/existing_user_controller.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.cc',
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager.cc',
            'components/signin/core/browser/refresh_token_annotation_request.cc',
            'components/signin/core/browser/account_reconcilor.cc',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate.mm',
            'chrome/browser/signin/dice_response_handler.cc',
            'components/signin/core/browser/test_signin_client.h',
            'components/signin/core/browser/account_tracker_service.cc'
        ],
        'components/signin/core/browser/signin_error_controller.h': [
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.h',
            'chrome/browser/ui/avatar_button_error_controller.h',
            'components/signin/core/browser/signin_error_controller_unittest.cc',
            'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'components/signin/core/browser/about_signin_internals.h',
            'ios/web_view/internal/signin/web_view_signin_error_controller_factory.cc',
            'chrome/browser/signin/signin_global_error.h',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'chrome/browser/signin/chrome_signin_client.h',
            'chrome/browser/signin/signin_error_notifier_ash_unittest.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate_unittest.cc',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate.h',
            'ios/chrome/browser/signin/signin_capability.cc',
            'components/signin/core/browser/signin_error_controller.cc',
            'chrome/browser/signin/signin_error_notifier_ash.h',
            'components/signin/ios/browser/ios_signin_client.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/sync/sync_ui_util.cc',
            'components/signin/core/browser/fake_auth_status_provider.h',
            'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util.cc',
            'chrome/browser/app_controller_mac_unittest.mm',
            'chrome/browser/ui/webui/settings/people_handler.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler.cc',
            'chrome/browser/ui/webui/signin/signin_create_profile_handler.cc',
            'ios/chrome/browser/signin/signin_error_controller_factory.cc',
            'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc',
            'chrome/browser/ui/avatar_button_error_controller.cc',
            'chrome/browser/chromeos/login/signin/auth_sync_observer.h'
        ],
        'components/signin/core/browser/signin_internals_util.h': [
            'components/signin/core/browser/signin_internals_util.cc',
            'components/signin/core/browser/about_signin_internals.cc',
            'components/signin/core/browser/about_signin_internals.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager.cc',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/signin_manager.h': [
            'chrome/browser/ui/webui/signin/login_ui_service_factory.cc',
            'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api_unittest.cc',
            'components/gcm_driver/gcm_profile_service.cc',
            'chrome/browser/ui/webui/signin/sync_confirmation_handler.cc',
            'chrome/browser/android/bookmarks/bookmark_bridge.cc',
            'chrome/browser/signin/dice_browsertest.cc',
            'components/browser_sync/profile_sync_service_typed_url_unittest.cc',
            'chrome/browser/chromeos/policy/device_local_account_browsertest.cc',
            'chrome/browser/ui/webui/print_preview/privet_printer_handler.cc',
            'components/signin/core/browser/fake_signin_manager.h',
            'chrome/browser/apps/drive/drive_service_bridge.cc',
            'chrome/browser/extensions/api/feedback_private/chrome_feedback_private_delegate.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc',
            'chrome/browser/chromeos/extensions/file_manager/private_api_misc.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_base.h',
            'chrome/browser/sync/test/integration/sync_test.cc',
            'chrome/browser/signin/chrome_signin_status_metrics_provider_delegate.cc',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_service_factory.cc',
            'chrome/browser/chrome_browser_main_android.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_sync_service.cc',
            'chrome/browser/ui/webui/profile_info_watcher.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc',
            'chrome/browser/gcm/gcm_profile_service_unittest.cc',
            'chrome/browser/autocomplete/contextual_suggestions_service_factory.cc',
            'chrome/browser/ui/search/search_tab_helper.cc',
            'components/browser_sync/profile_sync_service_startup_unittest.cc',
            'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service.cc',
            'ios/chrome/browser/ui/ntp/recent_tabs/recent_tabs_table_coordinator_unittest.mm',
            'ios/chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm',
            'chrome/browser/chromeos/login/lock/screen_locker.cc',
            'chrome/browser/ui/desktop_ios_promotion/sms_service.cc',
            'components/history/core/browser/web_history_service.cc',
            'components/ntp_snippets/remote/json_request.cc',
            'chrome/browser/signin/force_signin_verifier.cc',
            'ios/chrome/browser/autofill/personal_data_manager_factory.cc',
            'chrome/browser/password_manager/password_store_signin_notifier_impl.cc',
            'ios/chrome/browser/ui/bookmarks/bookmark_promo_controller.mm',
            'chrome/browser/first_run/first_run.cc',
            'chrome/browser/supervised_user/supervised_user_service.cc',
            'ios/chrome/browser/ui/settings/settings_collection_view_controller.mm',
            'chrome/browser/ui/views/profiles/avatar_button.cc',
            'chrome/browser/history/web_history_service_factory.cc',
            'chrome/browser/profiles/gaia_info_update_service.h',
            'components/ntp_snippets/remote/remote_suggestions_status_service_impl.cc',
            'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc',
            'chrome/browser/signin/about_signin_internals_factory.cc',
            'chrome/browser/ui/webui/settings/settings_manage_profile_handler.cc',
            'chrome/browser/profiles/profile_downloader.cc',
            'components/password_manager/sync/browser/password_sync_util.h',
            'chrome/browser/extensions/external_component_loader.cc',
            'chrome/browser/ui/desktop_ios_promotion/sms_service_factory.cc',
            'chrome/browser/extensions/api/identity/identity_apitest.cc',
            'chrome/browser/signin/signin_global_error_unittest.cc',
            'chrome/browser/extensions/api/signed_in_devices/signed_in_devices_manager_unittest.cc',
            'chrome/browser/ui/views/profiles/forced_reauthentication_dialog_view_browsertest.cc',
            'chrome/browser/extensions/api/enterprise_platform_keys/enterprise_platform_keys_api_unittest.cc',
            'components/signin/core/browser/about_signin_internals.cc',
            'chrome/browser/android/signin/signin_manager_android.cc',
            'chrome/browser/ui/sync/one_click_signin_sync_observer_unittest.cc',
            'components/signin/core/browser/about_signin_internals.h',
            'chrome/browser/ui/app_list/speech_auth_helper.cc',
            'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util_unittest.cc',
            'chrome/browser/signin/account_investigator_factory.cc',
            'chrome/browser/autocomplete/search_provider_unittest.cc',
            'chrome/browser/signin/process_dice_header_observer_impl.cc',
            'ios/chrome/browser/ui/first_run/first_run_util.mm',
            'chrome/browser/extensions/api/screenlock_private/screenlock_private_apitest.cc',
            'chrome/browser/browsing_data/sync_aware_counter_browsertest.cc',
            'chrome/browser/policy/cloud/cloud_policy_manager_browsertest.cc',
            'chrome/browser/ui/webui/settings/people_handler_unittest.cc',
            'chrome/browser/ui/webui/print_preview/print_preview_handler.cc',
            'chrome/browser/ui/cocoa/bookmarks/bookmark_bubble_controller_unittest.mm',
            'chrome/browser/ui/views/profiles/forced_reauthentication_dialog_view.cc',
            'chrome/test/data/webui/ntp4_browsertest.cc',
            'chrome/browser/signin/signin_manager_factory.cc',
            'chrome/browser/ui/webui/settings/settings_clear_browsing_data_handler.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/ui/views/profiles/profile_chooser_view.cc',
            'chrome/browser/android/data_usage/data_use_ui_tab_model_factory.cc',
            'components/signin/core/browser/account_reconcilor.h',
            'ios/chrome/browser/suggestions/suggestions_service_factory.mm',
            'chrome/browser/extensions/api/hotword_private/hotword_private_apitest.cc',
            'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc',
            'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc',
            'chrome/browser/sync/profile_sync_service_factory.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'chrome/browser/signin/signin_error_notifier_ash_unittest.cc',
            'chrome/browser/app_controller_mac.mm',
            'ios/chrome/browser/ui/tab_switcher/tab_switcher_model.mm',
            'chrome/browser/profiles/profile_manager.cc',
            'ios/chrome/browser/ntp_snippets/ios_chrome_content_suggestions_service_factory_util.cc',
            'ios/chrome/browser/ui/settings/signin_settings_egtest.mm',
            'chrome/browser/safe_browsing/chrome_password_protection_service.cc',
            'chrome/browser/ui/toolbar/app_menu_model.cc',
            'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm',
            'chrome/browser/chromeos/login/session/chrome_session_manager.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_mobile.cc',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder_factory.cc',
            'chrome/browser/ui/extensions/extension_installed_bubble_browsertest.cc',
            'chrome/browser/autofill/personal_data_manager_factory.cc',
            'chrome/browser/signin/signin_ui_util.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service.cc',
            'ios/chrome/browser/autofill/autofill_controller.mm',
            'chrome/browser/ntp_snippets/contextual_content_suggestions_service_factory.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.h',
            'ios/chrome/browser/invalidation/ios_chrome_profile_invalidation_provider_factory.mm',
            'ios/chrome/browser/ui/authentication/signin_interaction_controller_egtest.mm',
            'ios/chrome/browser/ui/payments/payment_request_mediator_unittest.mm',
            'chrome/browser/policy/cloud/component_cloud_policy_browsertest.cc',
            'components/signin/ios/browser/account_consistency_service.h',
            'components/ntp_snippets/content_suggestions_service.h',
            'chrome/browser/password_manager/chrome_password_manager_client.cc',
            'chrome/browser/invalidation/profile_invalidation_provider_factory.cc',
            'ios/web_view/internal/signin/cwv_authentication_controller.mm',
            'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc',
            'components/autofill/core/browser/personal_data_manager.cc',
            'chrome/browser/signin/easy_unlock_service.cc',
            'chrome/browser/ui/chrome_pages.cc',
            'chrome/browser/ui/toolbar/media_router_contextual_menu.cc',
            'chrome/browser/gcm/gcm_profile_service_factory.cc',
            'ios/chrome/browser/signin/signin_capability.cc',
            'chrome/browser/extensions/api/identity/identity_api.cc',
            'ios/chrome/browser/ui/settings/privacy_collection_view_controller.mm',
            'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder.cc',
            'chrome/browser/chromeos/first_run/first_run.cc',
            'chrome/browser/supervised_user/supervised_user_google_auth_navigation_throttle.cc',
            'chrome/browser/sync/profile_sync_test_util.cc',
            'chrome/browser/sync/sync_ui_util_unittest.cc',
            'components/signin/core/browser/signin_status_metrics_provider.cc',
            'ios/chrome/browser/signin/ios_chrome_signin_status_metrics_provider_delegate.cc',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'chrome/browser/search/suggestions/suggestions_service_factory.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service.cc',
            'chrome/browser/chromeos/login/kiosk_browsertest.cc',
            'ios/chrome/browser/ui/first_run/first_run_egtest.mm',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc',
            'chrome/browser/supervised_user/child_accounts/child_account_service.cc',
            'ios/chrome/app/main_controller.mm',
            'ios/chrome/browser/payments/payment_request.mm',
            'chrome/browser/signin/signin_tracker_unittest.cc',
            'components/signin/core/browser/signin_tracker.h',
            'chrome/browser/sync/profile_sync_service_android.cc',
            'ios/chrome/browser/sync/synced_sessions_bridge.mm',
            'ios/chrome/browser/services/gcm/ios_chrome_gcm_profile_service_factory.cc',
            'chrome/browser/payments/chrome_payment_request_delegate.cc',
            'chrome/browser/chromeos/login/signin/token_handle_fetcher.cc',
            'chrome/browser/ui/webui/ntp/ntp_resource_cache.cc',
            'ios/web_view/internal/signin/web_view_signin_manager_factory.cc',
            'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api.cc',
            'chrome/browser/chromeos/drive/drive_integration_service.cc',
            'chrome/browser/ntp_snippets/content_suggestions_service_factory.cc',
            'chrome/browser/devtools/device/devtools_android_bridge.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc',
            'chrome/browser/policy/cloud/user_policy_signin_service_base.cc',
            'chrome/browser/signin/signin_global_error.cc',
            'chrome/browser/extensions/extension_gcm_app_handler_unittest.cc',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'chrome/browser/extensions/api/webstore_private/webstore_private_api.cc',
            'chrome/browser/policy/cloud/cloud_policy_browsertest.cc',
            'chrome/browser/ui/webui/app_launcher_login_handler.cc',
            'chrome/browser/ui/startup/startup_tab_provider.cc',
            'chrome/browser/signin/easy_unlock_service_regular.cc',
            'chrome/browser/sync_file_system/drive_backend/sync_engine.cc',
            'chrome/browser/signin/signin_promo_util.cc',
            'chrome/browser/app_controller_mac_unittest.mm',
            'chrome/browser/chromeos/app_mode/startup_app_launcher.cc',
            'chrome/browser/safe_browsing/permission_reporter_browsertest.cc',
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller.mm',
            'chrome/browser/chromeos/extensions/file_manager/private_api_drive.cc',
            'chrome/browser/ui/webui/settings/people_handler.cc',
            'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler.cc',
            'chrome/browser/extensions/updater/chrome_extension_downloader_factory.cc',
            'chrome/browser/profiles/profile_impl.cc',
            'chrome/browser/profiles/profile_window.cc',
            'chrome/browser/chromeos/file_manager/file_manager_browsertest.cc',
            'chrome/browser/ui/views/payments/cvc_unmask_view_controller.cc',
            'components/signin/core/browser/signin_manager.cc',
            'components/browser_sync/profile_sync_service.cc',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc',
            'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_unittest.cc',
            'chrome/browser/ui/webui/media_router/media_router_webui_message_handler.cc',
            'chrome/browser/signin/signin_promo.cc',
            'chrome/browser/signin/dice_response_handler.cc',
            'ios/chrome/browser/ui/authentication/signin_interaction_controller.mm',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'components/password_manager/sync/browser/sync_credentials_filter.h',
            'chrome/browser/ui/webui/welcome_handler.cc',
            'ios/chrome/browser/ui/authentication/signin_earlgrey_utils.mm',
            'chrome/browser/feedback/feedback_uploader_chrome.cc',
            'ios/chrome/browser/ui/authentication/authentication_flow_performer.mm',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_test_util.cc',
            'ios/chrome/browser/history/web_history_service_factory.cc',
            'ios/chrome/browser/payments/payment_request_unittest_base.mm',
            'ios/chrome/browser/ui/settings/accounts_collection_egtest.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm',
            'chrome/browser/ui/sync/one_click_signin_sync_starter.cc',
            'chrome/browser/supervised_user/supervised_user_service_unittest.cc',
            'chrome/browser/browsing_data/history_counter_browsertest.cc',
            'chrome/browser/ui/webui/settings/search_engines_handler.cc',
            'ios/chrome/browser/signin/authentication_service.mm',
            'chrome/browser/ui/webui/md_history_ui.cc',
            'components/signin/core/browser/account_tracker_service.cc',
            'components/invalidation/impl/gcm_invalidation_bridge.cc'
        ],
        'components/signin/core/browser/signin_manager_base.h': [
            'components/browser_sync/profile_sync_service.h',
            'components/sync/driver/signin_manager_wrapper.cc',
            'chrome/browser/chromeos/login/signin/auth_sync_observer.cc',
            'ios/chrome/browser/passwords/ios_chrome_password_manager_client.mm',
            'components/ntp_snippets/remote/json_request.cc',
            'components/signin/core/browser/signin_manager_base.cc',
            'chrome/browser/signin/easy_unlock_service_unittest_chromeos.cc',
            'ios/web_view/internal/app/application_context.cc',
            'chrome/browser/supervised_user/supervised_user_service.cc',
            'chrome/browser/chromeos/arc/auth/arc_auth_context.cc',
            'chrome/browser/extensions/external_provider_impl_chromeos_unittest.cc',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/chromeos/login/session/user_session_manager.cc',
            'components/signin/core/browser/account_investigator.cc',
            'chrome/browser/cryptauth/chrome_cryptauth_service.h',
            'chrome/browser/sync_file_system/drive_backend/sync_engine.h',
            'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc',
            'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc',
            'chrome/browser/sync/test/integration/profile_sync_service_harness.cc',
            'chrome/browser/android/signin/signin_manager_android.h',
            'components/ntp_snippets/breaking_news/subscription_manager_impl.h',
            'services/identity/identity_manager.h',
            'components/sync/driver/about_sync_util.cc',
            'chrome/browser/password_manager/password_store_signin_notifier_impl.h',
            'chrome/browser/ui/webui/settings/people_handler.h',
            'components/password_manager/sync/browser/sync_username_test_base.h',
            'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc',
            'components/suggestions/suggestions_service_impl.cc',
            'components/signin/core/browser/signin_status_metrics_provider.h',
            'components/ntp_snippets/remote/remote_suggestions_fetcher_impl.cc',
            'chrome/browser/download/notification/download_notification_browsertest.cc',
            'chrome/browser/supervised_user/legacy/supervised_user_sync_service.h',
            'chrome/browser/sync/sync_ui_util.cc',
            'components/signin/core/browser/access_token_fetcher.h',
            'chrome/browser/printing/cloud_print/privet_notifications.cc',
            'chrome/browser/ui/webui/settings/people_handler.cc',
            'components/ntp_snippets/breaking_news/subscription_manager_impl.cc',
            'ios/chrome/browser/sync/synced_sessions_bridge.h',
            'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc',
            'chrome/browser/sync/supervised_user_signin_manager_wrapper.cc',
            'components/signin/core/browser/profile_identity_provider.h',
            'chrome/browser/metrics/ukm_browsertest.cc',
            'services/identity/identity_service.h',
            'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc',
            'components/browser_sync/profile_sync_test_util.cc',
            'chrome/browser/signin/chrome_proximity_auth_client.cc'
        ],
        'components/signin/core/browser/test_signin_client.h': [
            'chrome/browser/ui/desktop_ios_promotion/sms_service_unittest.cc',
            'chrome/browser/signin/test_signin_client_builder.cc',
            'components/signin/core/browser/test_signin_client.cc',
            'components/signin/ios/browser/account_consistency_service_unittest.mm',
            'components/suggestions/suggestions_service_impl_unittest.cc',
            'chrome/browser/profiles/profile_downloader_unittest.cc',
            'components/browsing_data/core/history_notice_utils_unittest.cc',
            'components/ntp_snippets/breaking_news/subscription_manager_impl_unittest.cc',
            'chrome/browser/signin/account_reconcilor_unittest.cc',
            'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate_unittest.cc',
            'components/autofill/core/browser/autofill_metrics_unittest.cc',
            'components/signin/core/browser/account_tracker_service_unittest.cc',
            'services/identity/identity_manager_unittest.cc',
            'components/password_manager/sync/browser/sync_username_test_base.h',
            'chrome/browser/signin/signin_manager_unittest.cc',
            'components/browser_sync/profile_sync_test_util.h',
            'components/ntp_snippets/remote/remote_suggestions_status_service_impl_unittest.cc',
            'components/signin/core/browser/account_investigator_unittest.cc',
            'components/autofill/core/browser/personal_data_manager_unittest.cc',
            'components/signin/core/browser/access_token_fetcher_unittest.cc',
            'chrome/browser/search/one_google_bar/one_google_bar_service_unittest.cc',
            'components/ntp_snippets/remote/test_utils.cc',
            'components/signin/core/browser/gaia_cookie_manager_service_unittest.cc',
            'chrome/browser/signin/dice_response_handler_unittest.cc',
            'components/history/core/browser/web_history_service_unittest.cc',
            'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate_unittest.mm'
        ],
        'ios/chrome/browser/signin/account_tracker_service_factory.h': [
            'ios/chrome/browser/signin/about_signin_internals_factory.cc',
            'ios/chrome/browser/signin/signin_manager_factory.cc',
            'ios/chrome/browser/autofill/personal_data_manager_factory.cc',
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'ios/chrome/browser/ui/settings/sync_settings_collection_view_controller.mm',
            'ios/chrome/browser/ui/authentication/signed_in_accounts_view_controller.mm',
            'ios/chrome/browser/signin/authentication_service_factory.mm',
            'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm',
            'ios/chrome/browser/signin/oauth2_token_service_factory.mm',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'ios/chrome/test/app/signin_test_util.mm',
            'ios/chrome/browser/browser_state/browser_state_keyed_service_factories.mm',
            'ios/chrome/browser/signin/account_fetcher_service_factory.cc',
            'ios/chrome/browser/signin/account_tracker_service_factory.cc',
            'ios/chrome/browser/ui/authentication/authentication_flow_performer.mm',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm',
            'ios/chrome/browser/signin/authentication_service.mm'
        ],
        'ios/chrome/browser/signin/fake_oauth2_token_service_builder.h': [
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller_unittest.mm',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm'
        ],
        'ios/chrome/browser/signin/fake_signin_manager_builder.h': [
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller_unittest.mm',
            'ios/chrome/browser/payments/payment_request_unittest_base.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm'
        ],
        'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h': [
            'ios/chrome/browser/signin/account_consistency_service_factory.mm',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.cc',
            'ios/chrome/browser/signin/about_signin_internals_factory.cc',
            'ios/chrome/browser/signin/account_reconcilor_factory.cc',
            'ios/chrome/browser/signin/signin_manager_factory.cc',
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'ios/chrome/browser/browser_state/browser_state_keyed_service_factories.mm',
            'ios/chrome/browser/ui/webui/signin_internals_ui_ios.cc'
        ],
        'ios/chrome/browser/signin/ios_chrome_signin_client.h': [
            'ios/chrome/browser/signin/signin_client_factory.cc',
            'ios/chrome/browser/signin/ios_chrome_signin_client.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm'
        ],
        'ios/chrome/browser/signin/signin_error_controller_factory.h': [
            'ios/chrome/browser/signin/about_signin_internals_factory.cc',
            'ios/chrome/browser/signin/signin_client_factory.cc',
            'ios/chrome/browser/signin/signin_capability.cc',
            'ios/chrome/browser/signin/oauth2_token_service_factory.mm',
            'ios/chrome/browser/browser_state/browser_state_keyed_service_factories.mm',
            'ios/chrome/browser/signin/signin_error_controller_factory.cc',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm'
        ],
        'ios/chrome/browser/signin/signin_manager_factory.h': [
            'ios/chrome/browser/signin/account_consistency_service_factory.mm',
            'ios/chrome/browser/signin/about_signin_internals_factory.cc',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_service_factory.cc',
            'ios/chrome/browser/signin/account_reconcilor_factory.cc',
            'ios/chrome/browser/ui/ntp/recent_tabs/recent_tabs_table_coordinator_unittest.mm',
            'ios/chrome/browser/signin/signin_manager_factory.cc',
            'ios/chrome/browser/passwords/ios_chrome_password_manager_client.mm',
            'ios/chrome/browser/autofill/personal_data_manager_factory.cc',
            'ios/chrome/browser/ui/bookmarks/bookmark_promo_controller.mm',
            'ios/chrome/browser/signin/fake_signin_manager_builder.cc',
            'ios/chrome/browser/ui/settings/settings_collection_view_controller.mm',
            'ios/chrome/browser/ui/first_run/first_run_util.mm',
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller_unittest.mm',
            'ios/chrome/browser/suggestions/suggestions_service_factory.mm',
            'ios/chrome/browser/ui/tab_switcher/tab_switcher_model.mm',
            'ios/chrome/browser/ntp_snippets/ios_chrome_content_suggestions_service_factory_util.cc',
            'ios/chrome/browser/ui/settings/signin_settings_egtest.mm',
            'ios/chrome/browser/signin/authentication_service_factory.mm',
            'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm',
            'ios/chrome/browser/autofill/autofill_controller.mm',
            'ios/chrome/browser/prefs/browser_prefs.mm',
            'ios/chrome/browser/invalidation/ios_chrome_profile_invalidation_provider_factory.mm',
            'ios/chrome/browser/ui/authentication/signin_interaction_controller_egtest.mm',
            'ios/chrome/browser/ntp_snippets/ios_chrome_content_suggestions_service_factory.cc',
            'ios/chrome/browser/signin/signin_capability.cc',
            'ios/chrome/browser/signin/ios_chrome_signin_status_metrics_provider_delegate.cc',
            'ios/chrome/browser/ui/first_run/first_run_egtest.mm',
            'ios/chrome/app/main_controller.mm',
            'ios/chrome/browser/payments/payment_request.mm',
            'ios/chrome/browser/sync/synced_sessions_bridge.mm',
            'ios/chrome/browser/services/gcm/ios_chrome_gcm_profile_service_factory.cc',
            'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc',
            'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller.mm',
            'ios/chrome/browser/browser_state/browser_state_keyed_service_factories.mm',
            'ios/chrome/browser/ui/authentication/signin_interaction_controller.mm',
            'ios/chrome/browser/ui/authentication/signin_earlgrey_utils.mm',
            'ios/chrome/browser/ui/authentication/authentication_flow_performer.mm',
            'ios/chrome/browser/sync/ios_chrome_profile_sync_test_util.cc',
            'ios/chrome/browser/history/web_history_service_factory.cc',
            'ios/chrome/browser/payments/payment_request_unittest_base.mm',
            'ios/chrome/browser/ui/settings/accounts_collection_egtest.mm',
            'ios/chrome/browser/signin/authentication_service_unittest.mm',
            'ios/chrome/browser/signin/authentication_service.mm'
        ]
    },
    'including_to_included': {
        'chrome/browser/android/bookmarks/bookmark_bridge.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/android/data_usage/data_use_ui_tab_model_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/android/profiles/profile_downloader_android.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'chrome/browser/android/signin/account_tracker_service_android.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'chrome/browser/android/signin/signin_investigator_android.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'chrome/browser/android/signin/signin_manager_android.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/android/signin/signin_manager_android.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'chrome/browser/app_controller_mac.mm': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/app_controller_mac_unittest.mm': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/apps/drive/drive_service_bridge.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/autocomplete/contextual_suggestions_service_factory.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/autocomplete/search_provider_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/autofill/personal_data_manager_factory.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/browsing_data/history_counter_browsertest.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/browsing_data/sync_aware_counter_browsertest.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chrome_browser_main_android.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/app_mode/startup_app_launcher.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/arc/arc_session_manager_browsertest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/chromeos/arc/auth/arc_auth_context.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/arc/auth/arc_auth_service_browsertest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/chromeos/arc/auth/arc_background_auth_code_fetcher.cc':
            ['components/signin/core/browser/profile_oauth2_token_service.h'],
        'chrome/browser/chromeos/arc/policy/arc_android_management_checker.cc':
            ['components/signin/core/browser/profile_oauth2_token_service.h'],
        'chrome/browser/chromeos/drive/drive_integration_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/extensions/file_manager/private_api_drive.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/chromeos/extensions/file_manager/private_api_misc.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/file_manager/file_manager_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/first_run/first_run.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/existing_user_controller.cc': [
            'components/signin/core/browser/signin_client.h'
        ],
        'chrome/browser/chromeos/login/kiosk_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/lock/screen_locker.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/session/chrome_session_manager.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/session/user_session_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/auth_sync_observer.cc': [
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/auth_sync_observer.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/chromeos/login/signin/auth_sync_observer_factory.cc': [
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/device_id_browsertest.cc': [
            'chrome/browser/signin/chrome_signin_client_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/oauth2_browsertest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_error_controller.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/oauth2_login_manager.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/signin/oauth2_login_manager_factory.cc':
            [
                'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/chromeos/login/signin/oauth2_login_verifier.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'chrome/browser/chromeos/login/signin/token_handle_fetcher.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/login/users/avatar/user_image_manager_browsertest.cc':
            ['chrome/browser/signin/account_tracker_service_factory.h'],
        'chrome/browser/chromeos/policy/android_management_client_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'chrome/browser/chromeos/policy/device_local_account_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/chromeos/policy/user_cloud_policy_manager_chromeos_unittest.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/chromeos/policy/user_cloud_policy_token_forwarder_factory.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/cryptauth/chrome_cryptauth_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/cryptauth/chrome_cryptauth_service.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'chrome/browser/cryptauth/chrome_cryptauth_service_factory.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/devtools/device/devtools_android_bridge.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/download/notification/download_notification_browsertest.cc':
            [
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/enterprise_platform_keys/enterprise_platform_keys_api_unittest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/enterprise_platform_keys_private/enterprise_platform_keys_private_api_unittest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/feedback_private/chrome_feedback_private_delegate.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/hotword_private/hotword_private_apitest.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/identity/gaia_web_auth_flow.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/extensions/api/identity/identity_api.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/extensions/api/identity/identity_apitest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/extensions/api/identity/identity_get_auth_token_function.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/chrome_signin_client_factory.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h'
            ],
        'chrome/browser/extensions/api/screenlock_private/screenlock_private_apitest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/signed_in_devices/signed_in_devices_manager_unittest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/api/sync_file_system/sync_file_system_browsertest.cc':
            ['chrome/browser/signin/fake_signin_manager_builder.h'],
        'chrome/browser/extensions/api/webstore_private/webstore_private_api.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/extension_gcm_app_handler_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/extensions/extension_system_factory.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/extensions/external_component_loader.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/extensions/external_provider_impl_chromeos_unittest.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/extensions/updater/chrome_extension_downloader_factory.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/feedback/feedback_uploader_chrome.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/first_run/first_run.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/gcm/gcm_profile_service_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/gcm/gcm_profile_service_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/history/web_history_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/invalidation/profile_invalidation_provider_factory.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/metrics/ukm_browsertest.cc': [
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ntp_snippets/content_suggestions_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ntp_snippets/contextual_content_suggestions_service_factory.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/password_manager/chrome_password_manager_client.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/password_manager/password_store_factory.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/password_manager/password_store_signin_notifier_impl.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/password_manager/password_store_signin_notifier_impl.h':
            ['components/signin/core/browser/signin_manager_base.h'],
        'chrome/browser/password_manager/password_store_signin_notifier_impl_unittest.cc':
            [
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/payments/chrome_payment_request_delegate.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/permissions/permission_uma_util_unittest.cc': [
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/policy/cloud/cloud_policy_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/policy/cloud/cloud_policy_manager_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/policy/cloud/component_cloud_policy_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service_base.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service_base.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service_factory.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service_mobile.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/policy/cloud/user_policy_signin_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/prefs/browser_prefs.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/printing/cloud_print/privet_notifications.cc': [
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/chrome_browser_main_extra_parts_profiles.cc': [
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/gaia_info_update_service.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/gaia_info_update_service.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/profiles/gaia_info_update_service_factory.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/gaia_info_update_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profile_downloader.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profile_downloader.h': [
            'components/signin/core/browser/account_tracker_service.h'
        ],
        'chrome/browser/profiles/profile_downloader_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/test_signin_client.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/profiles/profile_impl.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profile_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profile_window.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profile_window_browsertest.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/profiles/profiles_state.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/safe_browsing/chrome_password_protection_service.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/safe_browsing/chrome_password_protection_service_unittest.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/signin_manager.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/chrome_signin_client_factory.h',
                'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/safe_browsing/permission_reporter_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/search/one_google_bar/one_google_bar_service.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'chrome/browser/search/one_google_bar/one_google_bar_service_factory.cc':
            ['chrome/browser/signin/gaia_cookie_manager_service_factory.h'],
        'chrome/browser/search/one_google_bar/one_google_bar_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'chrome/browser/search/suggestions/suggestions_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/about_signin_internals_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/account_fetcher_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/signin/account_investigator_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/account_reconcilor_factory.cc': [
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/account_reconcilor_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/test_signin_client.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/signin/account_tracker_service_factory.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h'
        ],
        'chrome/browser/signin/chrome_proximity_auth_client.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_client.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/chrome_signin_client.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_client.h': [
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/signin/chrome_signin_client_factory.cc': [
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_client_factory.h': [
            'chrome/browser/signin/chrome_signin_client.h'
        ],
        'chrome/browser/signin/chrome_signin_client_unittest.cc': [
            'chrome/browser/signin/chrome_signin_client.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_helper.cc': [
            'chrome/browser/signin/chrome_signin_client.h',
            'chrome/browser/signin/chrome_signin_client_factory.h'
        ],
        'chrome/browser/signin/chrome_signin_status_metrics_provider_delegate.cc':
            ['components/signin/core/browser/signin_manager.h'],
        'chrome/browser/signin/chrome_signin_status_metrics_provider_delegate.h':
            ['chrome/browser/signin/signin_manager_factory.h'],
        'chrome/browser/signin/dice_browsertest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/dice_response_handler.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/dice_response_handler_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'chrome/browser/signin/easy_unlock_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/easy_unlock_service_regular.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/easy_unlock_service_unittest_chromeos.cc': [
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/fake_account_fetcher_service_builder.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/fake_gaia_cookie_manager_service_builder.h'
        ],
        'chrome/browser/signin/fake_profile_oauth2_token_service_builder.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/signin/fake_signin_manager_builder.cc': [
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/signin/fake_signin_manager_builder.h': [
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'chrome/browser/signin/force_signin_verifier.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/gaia_cookie_manager_service_factory.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.cc':
            ['components/signin/core/browser/signin_client.h'],
        'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate.h':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_error_controller.h'
            ],
        'chrome/browser/signin/mutable_profile_oauth2_token_service_delegate_unittest.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_error_controller.h',
                'components/signin/core/browser/test_signin_client.h'
            ],
        'chrome/browser/signin/oauth2_token_service_delegate_android.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/signin/oauth2_token_service_delegate_android.h': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'chrome/browser/signin/process_dice_header_observer_impl.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/profile_oauth2_token_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/signin_error_controller_factory.cc': [
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/signin_error_controller_factory.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/signin/signin_error_notifier_ash.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/signin/signin_error_notifier_ash_unittest.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/signin/signin_error_notifier_factory_ash.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/signin_global_error.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/signin_global_error.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/signin/signin_global_error_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'chrome/browser/signin/signin_global_error_unittest.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/signin/signin_manager_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/signin_manager_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/test_signin_client.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/signin/signin_promo.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/signin_promo_util.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/signin_tracker_factory.cc': [
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/signin_tracker_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/signin/signin_ui_util.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/signin/test_signin_client_builder.cc': [
            'components/signin/core/browser/test_signin_client.h'
        ],
        'chrome/browser/supervised_user/child_accounts/child_account_service.cc':
            [
                'components/signin/core/browser/gaia_cookie_manager_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/child_accounts/child_account_service.h':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/gaia_cookie_manager_service.h'
            ],
        'chrome/browser/supervised_user/child_accounts/child_account_service_factory.cc':
            [
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/child_accounts/family_info_fetcher_unittest.cc':
            [
                'components/signin/core/browser/fake_profile_oauth2_token_service.h'
            ],
        'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/child_accounts/permission_request_creator_apiary_unittest.cc':
            [
                'components/signin/core/browser/fake_profile_oauth2_token_service.h'
            ],
        'chrome/browser/supervised_user/experimental/safe_search_url_reporter.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/experimental/safe_search_url_reporter_unittest.cc':
            [
                'components/signin/core/browser/fake_profile_oauth2_token_service.h'
            ],
        'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/legacy/custodian_profile_downloader_service_factory.cc':
            [
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/legacy/supervised_user_refresh_token_fetcher_unittest.cc':
            [
                'components/signin/core/browser/fake_profile_oauth2_token_service.h'
            ],
        'chrome/browser/supervised_user/legacy/supervised_user_registration_utility.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_client.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/chrome_signin_client_factory.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/legacy/supervised_user_sync_service.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/legacy/supervised_user_sync_service.h':
            ['components/signin/core/browser/signin_manager_base.h'],
        'chrome/browser/supervised_user/legacy/supervised_user_sync_service_factory.cc':
            ['chrome/browser/signin/signin_manager_factory.h'],
        'chrome/browser/supervised_user/supervised_user_google_auth_navigation_throttle.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/supervised_user/supervised_user_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/supervised_user/supervised_user_service_factory.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/supervised_user/supervised_user_service_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/sync/chrome_sync_client.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/sync/profile_sync_service_android.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync/profile_sync_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync/profile_sync_test_util.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync/supervised_user_signin_manager_wrapper.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'chrome/browser/sync/sync_ui_util.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync/sync_ui_util_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync/test/integration/fake_server_invalidation_service.h': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'chrome/browser/sync/test/integration/profile_sync_service_harness.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h'
            ],
        'chrome/browser/sync/test/integration/sync_auth_test.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/sync/test/integration/sync_test.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync_file_system/drive_backend/sync_engine.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/sync_file_system/drive_backend/sync_engine.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'chrome/browser/ui/app_list/speech_auth_helper.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/app_list/speech_auth_helper.h': [
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'chrome/browser/ui/app_list/speech_auth_helper_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager_base.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/ui/ash/multi_user/multi_user_util_chromeos_unittest.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/ui/autofill/chrome_autofill_client.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/avatar_button_error_controller.cc': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/ui/avatar_button_error_controller.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'chrome/browser/ui/chrome_pages.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/cocoa/bookmarks/bookmark_bubble_controller_unittest.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/cocoa/profiles/profile_chooser_controller.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/cocoa/profiles/profile_chooser_controller_unittest.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h'
        ],
        'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util.cc':
            ['components/signin/core/browser/signin_error_controller.h'],
        'chrome/browser/ui/desktop_ios_promotion/desktop_ios_promotion_util_unittest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_error_controller_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/desktop_ios_promotion/sms_service.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/ui/desktop_ios_promotion/sms_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/desktop_ios_promotion/sms_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'chrome/browser/ui/extensions/extension_installed_bubble_browsertest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/passwords/password_manager_presenter.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/search/local_ntp_browsertest.cc': [
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h'
        ],
        'chrome/browser/ui/search/search_tab_helper.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/search/search_tab_helper_unittest.cc': [
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/startup/startup_tab_provider.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/sync/one_click_signin_sync_observer_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/sync/one_click_signin_sync_starter.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/sync/one_click_signin_sync_starter_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/sync/sync_promo_ui_unittest.cc': [
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/toolbar/app_menu_model.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/toolbar/app_menu_model_unittest.cc': [
            'chrome/browser/signin/profile_oauth2_token_service_factory.h'
        ],
        'chrome/browser/ui/toolbar/media_router_contextual_menu.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/toolbar/media_router_contextual_menu_unittest.cc': [
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_browsertest.cc':
            [
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/ui/views/bookmarks/bookmark_bubble_view_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/views/payments/cvc_unmask_view_controller.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/views/profiles/avatar_button.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/views/profiles/forced_reauthentication_dialog_view.cc':
            ['components/signin/core/browser/signin_manager.h'],
        'chrome/browser/ui/views/profiles/forced_reauthentication_dialog_view_browsertest.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/views/profiles/profile_chooser_view.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/app_launcher_login_handler.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/browsing_history_handler_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/webui/local_discovery/local_discovery_ui_browsertest.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'components/signin/core/browser/signin_manager_base.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.cc':
            [
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'chrome/browser/signin/profile_oauth2_token_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/webui/local_discovery/local_discovery_ui_handler.h':
            ['components/signin/core/browser/signin_manager.h'],
        'chrome/browser/ui/webui/md_history_ui.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/media_router/media_router_webui_message_handler.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/webui/ntp/ntp_resource_cache.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/ntp/ntp_resource_cache_factory.cc': [
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/print_preview/print_preview_handler.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/print_preview/print_preview_handler.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'chrome/browser/ui/webui/print_preview/privet_printer_handler.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/profile_info_watcher.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/settings/people_handler.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager_base.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/settings/people_handler.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'chrome/browser/ui/webui/settings/people_handler_unittest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/webui/settings/search_engines_handler.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/settings/settings_clear_browsing_data_handler.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/webui/settings/settings_manage_profile_handler.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/signin/inline_login_handler_impl.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_error_controller.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/signin/inline_login_ui_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h',
            'chrome/browser/signin/fake_profile_oauth2_token_service_builder.h',
            'chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'chrome/browser/ui/webui/signin/login_ui_service_factory.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'chrome/browser/ui/webui/signin/signin_create_profile_handler.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'chrome/browser/signin/signin_error_controller_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/signin/signin_create_profile_handler_unittest.cc':
            [
                'chrome/browser/signin/signin_error_controller_factory.h',
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/ui/webui/signin/signin_dice_internals_handler.cc': [
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler.cc':
            [
                'components/signin/core/browser/signin_error_controller.h',
                'components/signin/core/browser/signin_manager.h',
                'chrome/browser/signin/signin_error_controller_factory.h',
                'chrome/browser/signin/signin_manager_factory.h'
            ],
        'chrome/browser/ui/webui/signin/signin_supervised_user_import_handler_unittest.cc':
            [
                'chrome/browser/signin/signin_error_controller_factory.h',
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/ui/webui/signin/sync_confirmation_handler.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/browser/ui/webui/signin/sync_confirmation_handler.h': [
            'components/signin/core/browser/account_tracker_service.h'
        ],
        'chrome/browser/ui/webui/signin/sync_confirmation_handler_unittest.cc':
            [
                'components/signin/core/browser/fake_signin_manager.h',
                'chrome/browser/signin/account_tracker_service_factory.h',
                'chrome/browser/signin/signin_manager_factory.h',
                'chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'chrome/browser/ui/webui/signin_internals_ui.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h'
        ],
        'chrome/browser/ui/webui/supervised_user_internals_message_handler.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'chrome/browser/signin/account_tracker_service_factory.h'
            ],
        'chrome/browser/ui/webui/welcome_handler.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/test/base/chrome_render_view_host_test_harness.cc': [
            'components/signin/core/browser/fake_signin_manager.h',
            'chrome/browser/signin/account_tracker_service_factory.h',
            'chrome/browser/signin/chrome_signin_client_factory.h',
            'chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'chrome/browser/signin/profile_oauth2_token_service_factory.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'chrome/test/data/webui/ntp4_browsertest.cc': [
            'components/signin/core/browser/signin_manager.h',
            'chrome/browser/signin/signin_manager_factory.h'
        ],
        'components/autofill/core/browser/autofill_metrics_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/autofill/core/browser/autofill_test_utils.cc': [
            'components/signin/core/browser/account_tracker_service.h'
        ],
        'components/autofill/core/browser/personal_data_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/autofill/core/browser/personal_data_manager_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/browser_sync/profile_sync_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/browser_sync/profile_sync_service.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/browser_sync/profile_sync_service_startup_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'components/browser_sync/profile_sync_service_typed_url_unittest.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/browser_sync/profile_sync_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'components/browser_sync/profile_sync_test_util.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/browser_sync/profile_sync_test_util.h': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/browsing_data/core/history_notice_utils_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/gcm_driver/gcm_profile_service.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/history/core/browser/web_history_service.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/history/core/browser/web_history_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/invalidation/impl/fake_invalidation_service.h': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'components/invalidation/impl/gcm_invalidation_bridge.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/invalidation/impl/gcm_invalidation_bridge_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'components/ntp_snippets/breaking_news/subscription_manager_impl.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/ntp_snippets/breaking_news/subscription_manager_impl.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/ntp_snippets/breaking_news/subscription_manager_impl_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/ntp_snippets/content_suggestions_service.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/ntp_snippets/contextual/contextual_suggestions_fetcher_impl_unittest.cc':
            [
                'components/signin/core/browser/fake_profile_oauth2_token_service.h',
                'components/signin/core/browser/fake_signin_manager.h'
            ],
        'components/ntp_snippets/remote/json_request.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/ntp_snippets/remote/remote_suggestions_fetcher_impl.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/ntp_snippets/remote/remote_suggestions_fetcher_impl_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'components/ntp_snippets/remote/remote_suggestions_provider_impl_unittest.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'components/ntp_snippets/remote/remote_suggestions_status_service_impl.cc':
            ['components/signin/core/browser/signin_manager.h'],
        'components/ntp_snippets/remote/remote_suggestions_status_service_impl_unittest.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/fake_signin_manager.h',
                'components/signin/core/browser/test_signin_client.h'
            ],
        'components/ntp_snippets/remote/test_utils.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/ntp_snippets/remote/test_utils.h': [
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'components/password_manager/sync/browser/password_sync_util.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/password_manager/sync/browser/sync_credentials_filter.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/password_manager/sync/browser/sync_username_test_base.h': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_manager_base.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/signin/core/browser/about_signin_internals.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_internals_util.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/about_signin_internals.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_internals_util.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/access_token_fetcher.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/access_token_fetcher_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/signin/core/browser/account_fetcher_service.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/account_investigator.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/account_investigator.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'components/signin/core/browser/account_investigator_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/signin/core/browser/account_reconcilor.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/account_reconcilor.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/account_tracker_service.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/account_tracker_service_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/signin/core/browser/child_account_info_fetcher_android.cc':
            ['components/signin/core/browser/account_tracker_service.h'],
        'components/signin/core/browser/child_account_info_fetcher_impl.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/fake_account_fetcher_service.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/fake_auth_status_provider.h': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'components/signin/core/browser/fake_gaia_cookie_manager_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_gaia_cookie_manager_service.h'
        ],
        'components/signin/core/browser/fake_gaia_cookie_manager_service.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'components/signin/core/browser/fake_profile_oauth2_token_service.cc': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/fake_profile_oauth2_token_service.h': [
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/fake_signin_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_signin_manager.h'
        ],
        'components/signin/core/browser/fake_signin_manager.h': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/gaia_cookie_manager_service.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'components/signin/core/browser/gaia_cookie_manager_service.h': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/gaia_cookie_manager_service_unittest.cc':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/gaia_cookie_manager_service.h',
                'components/signin/core/browser/test_signin_client.h'
            ],
        'components/signin/core/browser/profile_identity_provider.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/profile_identity_provider.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/profile_oauth2_token_service.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/refresh_token_annotation_request.cc': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/signin_client.cc': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/signin_cookie_changed_subscription.h': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/core/browser/signin_error_controller.cc': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'components/signin/core/browser/signin_error_controller_unittest.cc': [
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'components/signin/core/browser/signin_internals_util.cc': [
            'components/signin/core/browser/signin_internals_util.h'
        ],
        'components/signin/core/browser/signin_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_internals_util.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/signin_manager.h': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_internals_util.h',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/signin_manager_base.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/signin_manager_base.h': [
            'components/signin/core/browser/signin_internals_util.h'
        ],
        'components/signin/core/browser/signin_status_metrics_provider.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/signin_status_metrics_provider.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/signin/core/browser/signin_tracker.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'components/signin/core/browser/signin_tracker.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/core/browser/test_signin_client.cc': [
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/signin/core/browser/test_signin_client.h': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/ios/browser/account_consistency_service.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'components/signin/ios/browser/account_consistency_service.mm': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/signin/ios/browser/account_consistency_service_unittest.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/gaia_cookie_manager_service.h',
                'components/signin/core/browser/fake_signin_manager.h',
                'components/signin/core/browser/test_signin_client.h'
            ],
        'components/signin/ios/browser/ios_signin_client.h': [
            'components/signin/core/browser/signin_client.h',
            'components/signin/core/browser/signin_error_controller.h'
        ],
        'components/signin/ios/browser/merge_session_observer_bridge.h': [
            'components/signin/core/browser/gaia_cookie_manager_service.h'
        ],
        'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate.h':
            ['components/signin/core/browser/signin_error_controller.h'],
        'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/signin_client.h'
            ],
        'components/signin/ios/browser/profile_oauth2_token_service_ios_delegate_unittest.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/test_signin_client.h'
            ],
        'components/suggestions/suggestions_service_impl.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/suggestions/suggestions_service_impl_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'components/sync/driver/about_sync_util.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'components/sync/driver/glue/sync_backend_host_impl.cc': [
            'components/signin/core/browser/signin_client.h'
        ],
        'components/sync/driver/signin_manager_wrapper.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'ios/chrome/app/main_controller.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/autofill/autofill_controller.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/autofill/personal_data_manager_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/browser_state/browser_state_keyed_service_factories.mm':
            [
                'ios/chrome/browser/signin/account_tracker_service_factory.h',
                'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
                'ios/chrome/browser/signin/signin_error_controller_factory.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/browser_state/chrome_browser_state_manager_impl.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/history/web_history_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/invalidation/ios_chrome_profile_invalidation_provider_factory.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ntp_snippets/ios_chrome_content_suggestions_service_factory.cc':
            ['ios/chrome/browser/signin/signin_manager_factory.h'],
        'ios/chrome/browser/ntp_snippets/ios_chrome_content_suggestions_service_factory_util.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/passwords/ios_chrome_password_manager_client.mm': [
            'components/signin/core/browser/signin_manager_base.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/payments/payment_request.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/payments/payment_request_unittest_base.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h',
            'ios/chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'ios/chrome/browser/prefs/browser_prefs.mm': [
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/services/gcm/ios_chrome_gcm_profile_service_factory.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/signin/about_signin_internals_factory.cc': [
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/account_consistency_service_factory.mm': [
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/account_fetcher_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'ios/chrome/browser/signin/account_reconcilor_factory.cc': [
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/account_tracker_service_factory.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'ios/chrome/browser/signin/authentication_service.mm': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/authentication_service_factory.mm': [
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/authentication_service_unittest.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/ios_chrome_signin_client.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.h',
            'ios/chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'ios/chrome/browser/signin/fake_oauth2_token_service_builder.mm': [
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h',
            'ios/chrome/browser/signin/fake_oauth2_token_service_builder.h'
        ],
        'ios/chrome/browser/signin/fake_signin_manager_builder.cc': [
            'components/signin/core/browser/fake_signin_manager.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h',
            'ios/chrome/browser/signin/fake_signin_manager_builder.h'
        ],
        'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h'
        ],
        'ios/chrome/browser/signin/ios_chrome_signin_client.mm': [
            'ios/chrome/browser/signin/ios_chrome_signin_client.h'
        ],
        'ios/chrome/browser/signin/ios_chrome_signin_status_metrics_provider_delegate.cc':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/signin/oauth2_token_service_factory.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'ios/chrome/browser/signin/signin_capability.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/signin/signin_client_factory.cc': [
            'ios/chrome/browser/signin/ios_chrome_signin_client.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'ios/chrome/browser/signin/signin_error_controller_factory.cc': [
            'components/signin/core/browser/signin_error_controller.h',
            'ios/chrome/browser/signin/signin_error_controller_factory.h'
        ],
        'ios/chrome/browser/signin/signin_manager_factory.cc': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/suggestions/suggestions_service_factory.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/sync/ios_chrome_profile_sync_service_factory.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/sync/ios_chrome_profile_sync_test_util.cc': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/sync/ios_chrome_sync_client.mm': [
            'components/signin/core/browser/profile_oauth2_token_service.h'
        ],
        'ios/chrome/browser/sync/synced_sessions_bridge.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'ios/chrome/browser/sync/synced_sessions_bridge.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/authentication/authentication_flow_performer.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/account_tracker_service_factory.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/authentication/signed_in_accounts_view_controller.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'ios/chrome/browser/signin/account_tracker_service_factory.h'
            ],
        'ios/chrome/browser/ui/authentication/signin_earlgrey_utils.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/authentication/signin_interaction_controller.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/authentication/signin_interaction_controller_egtest.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/bookmarks/bookmark_promo_controller.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/first_run/first_run_egtest.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/first_run/first_run_util.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/ntp/recent_tabs/recent_tabs_table_coordinator_unittest.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/payments/payment_request_mediator_unittest.mm': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'ios/chrome/browser/ui/settings/accounts_collection_egtest.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/settings/accounts_collection_view_controller.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/account_tracker_service_factory.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/settings/clear_browsing_data_collection_view_controller_unittest.mm':
            [
                'components/signin/core/browser/fake_signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h',
                'ios/chrome/browser/signin/fake_oauth2_token_service_builder.h',
                'ios/chrome/browser/signin/fake_signin_manager_builder.h'
            ],
        'ios/chrome/browser/ui/settings/privacy_collection_view_controller.mm':
            ['components/signin/core/browser/signin_manager.h'],
        'ios/chrome/browser/ui/settings/settings_collection_view_controller.mm':
            [
                'components/signin/core/browser/signin_manager.h',
                'ios/chrome/browser/signin/signin_manager_factory.h'
            ],
        'ios/chrome/browser/ui/settings/signin_settings_egtest.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/settings/sync_encryption_passphrase_collection_view_controller.mm':
            ['components/signin/core/browser/profile_oauth2_token_service.h'],
        'ios/chrome/browser/ui/settings/sync_settings_collection_view_controller.mm':
            [
                'components/signin/core/browser/account_tracker_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h',
                'ios/chrome/browser/signin/account_tracker_service_factory.h'
            ],
        'ios/chrome/browser/ui/tab_switcher/tab_switcher_model.mm': [
            'components/signin/core/browser/signin_manager.h',
            'ios/chrome/browser/signin/signin_manager_factory.h'
        ],
        'ios/chrome/browser/ui/webui/signin_internals_ui_ios.cc': [
            'components/signin/core/browser/gaia_cookie_manager_service.h',
            'ios/chrome/browser/signin/gaia_cookie_manager_service_factory.h'
        ],
        'ios/chrome/test/app/signin_test_util.mm': [
            'components/signin/core/browser/account_tracker_service.h',
            'ios/chrome/browser/signin/account_tracker_service_factory.h'
        ],
        'ios/web_view/internal/app/application_context.cc': [
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'ios/web_view/internal/signin/cwv_authentication_controller.mm': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager.h'
        ],
        'ios/web_view/internal/signin/web_view_account_fetcher_service_factory.cc':
            ['components/signin/core/browser/profile_oauth2_token_service.h'],
        'ios/web_view/internal/signin/web_view_account_tracker_service_factory.cc':
            ['components/signin/core/browser/account_tracker_service.h'],
        'ios/web_view/internal/signin/web_view_gaia_cookie_manager_service_factory.cc':
            [
                'components/signin/core/browser/gaia_cookie_manager_service.h',
                'components/signin/core/browser/profile_oauth2_token_service.h'
            ],
        'ios/web_view/internal/signin/web_view_oauth2_token_service_factory.mm':
            ['components/signin/core/browser/profile_oauth2_token_service.h'],
        'ios/web_view/internal/signin/web_view_signin_client_factory.cc': [
            'components/signin/core/browser/signin_client.h'
        ],
        'ios/web_view/internal/signin/web_view_signin_error_controller_factory.cc':
            ['components/signin/core/browser/signin_error_controller.h'],
        'ios/web_view/internal/signin/web_view_signin_manager_factory.cc': [
            'components/signin/core/browser/signin_manager.h'
        ],
        'services/identity/identity_manager.cc': [
            'components/signin/core/browser/account_tracker_service.h'
        ],
        'services/identity/identity_manager.h': [
            'components/signin/core/browser/profile_oauth2_token_service.h',
            'components/signin/core/browser/signin_manager_base.h'
        ],
        'services/identity/identity_manager_unittest.cc': [
            'components/signin/core/browser/account_tracker_service.h',
            'components/signin/core/browser/fake_profile_oauth2_token_service.h',
            'components/signin/core/browser/fake_signin_manager.h',
            'components/signin/core/browser/test_signin_client.h'
        ],
        'services/identity/identity_service.h': [
            'components/signin/core/browser/signin_manager_base.h'
        ]
    },
    'repo_commit_date': '2017-10-13',
    'repo_rev': '655ed5e8929d',
    'timestamp (UTC)': '2018-11-23 15:02:53.666201',
    'usage_analyzer_rev': 'ee51f71'
}
